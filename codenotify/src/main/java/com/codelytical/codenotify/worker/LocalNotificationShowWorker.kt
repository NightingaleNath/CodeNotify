package com.codelytical.codenotify.worker
...
import com.codelytical.codenotify.R
import com.codelytical.codenotify.datastore.AppDataStoreImpl
import com.codelytical.codenotify.extensions.COMPONENT_TYPE_FRAGMENT
import com.codelytical.codenotify.extensions.dataStore
import com.codelytical.codenotify.extensions.getBitmapFromURL
import com.codelytical.codenotify.extensions.getCroppedBitmap
...
class LocalNotificationShowWorker(
	...
	override suspend fun doWork(): Result {
		return try {
			...
			val targetComponentType = inputData.getString(LocalNotificationPostWorker.TARGET_COMPONENT_TYPE_KEY)
			val customObject = inputData.getString(LocalNotificationPostWorker.OBJECT_KEY)
			...
			val type =
				inputData.getInt(LocalNotificationPostWorker.TYPE_KEY, NotificationTypes.TEXT.type)

			val channelId =
				inputData.getString(LocalNotificationPostWorker.NOTIFICATION_CHANNEL_ID_KEY)!!
			val channelName =
				inputData.getString(LocalNotificationPostWorker.NOTIFICATION_CHANNEL_NAME_KEY)!!
	...
	private fun createNotification(
		...
		val intent = if (targetComponentType == COMPONENT_TYPE_FRAGMENT) {
			// Create an intent for a fragment
			Intent(Intent.ACTION_MAIN).apply {
				component = ComponentName(context.packageName, classPath)
				addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP)
				putExtra("fragment_target", true)
				putExtra("customObject", customObject)
			}
		} else {
